class ValueplusController < ApplicationController
  ...
  def index
    ...
    #분야별 추천수 상위 4개 집회
    @politic = @assembly.where(:category => 0).limit(1).order(like: :desc)
    @society = @assembly.where(:category => 1).limit(1).order(like: :desc)
    @education = @assembly.where(:category => 2).limit(1).order(like: :desc)
  ...
  def notchecked
    #미검토 시위
    @notchecked = Assembly.where(:check => 2)
  end
  def checked_yes
    #검토 승인한 시위
    @checked_yes = Assembly.where(:check => 1)
  end
  def checked_no
    #검토 거부한 시위
    @checked_no = Assembly.where(:check => 0)
  end
  #전체 리스팅 페이지
  def list
    # @assembly => 집회
